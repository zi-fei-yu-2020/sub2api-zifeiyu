...
func (s *AuthService) RefreshTokenPair(ctx context.Context, refreshToken string) (*TokenPairWithUser, error) {
	...
	if err != nil {
		if errors.Is(err, ErrRefreshTokenNotFound) {
			return nil, s.consumedRefreshTokenError(ctx, tokenHash, nil)
		}
	...
	rotation, err := s.refreshTokenCache.RotateRefreshToken(
		ctx,
		tokenHash,
		material.TokenHash,
		material.Data,
	...
	case RefreshTokenRotationReused:
		return nil, s.consumedRefreshTokenError(ctx, tokenHash, rotation.Consumed)
	default:
		return nil, ErrRefreshTokenInvalid
	}
}
...
func (s *AuthService) consumedRefreshTokenError(
	...
	if consumed == nil {
		var err error
		consumed, err = s.refreshTokenCache.GetConsumedRefreshToken(ctx, tokenHash)
		if err != nil {
			if errors.Is(err, ErrRefreshTokenNotFound) {
