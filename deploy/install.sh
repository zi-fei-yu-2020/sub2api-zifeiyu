...
declare -A MSG_ZH=(
    ...
    ["step4_open_wizard"]="安全打开首次安装向导："
    ["setup_local_only"]="首次安装向导默认仅监听服务器本机，不能直接通过公网 IP 访问。"
    ["setup_ssh_tunnel"]="请先在你的电脑上建立 SSH 隧道："
    ["setup_open_local"]="然后在本机浏览器打开："
...
declare -A MSG_EN=(
    ...
    ["step4_open_wizard"]="Open the first-run Setup Wizard securely:"
    ["setup_local_only"]="The first-run wizard listens on the server loopback interface by default and is not directly reachable through the public IP."
    ["setup_ssh_tunnel"]="First create an SSH tunnel from your computer:"
    ["setup_open_local"]="Then open this address in your local browser:"
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    print_warning "     $(msg 'setup_local_only')"
    echo ""
    echo "     $(msg 'setup_ssh_tunnel')"
    print_info "     ssh -L ${SERVER_PORT}:127.0.0.1:${SERVER_PORT} user@${display_host}"
    echo ""
    echo "     $(msg 'setup_open_local')"
    print_info "     http://127.0.0.1:${SERVER_PORT}/setup"
    echo ""
