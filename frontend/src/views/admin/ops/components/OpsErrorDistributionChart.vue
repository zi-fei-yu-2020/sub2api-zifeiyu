...
<template>
  <div class="flex h-full flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-slate-900 dark:ring-dark-700">
    <div class="mb-4 flex items-center justify-between">
      ...
      <button
        type="button"
        class="inline-flex items-center rounded-xl border border-slate-200 bg-white px-2 py-1 text-[11px] font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-800 dark:bg-dark-900 dark:text-gray-300 dark:hover:bg-dark-800"
    ...
    <div class="relative min-h-0 flex-1">
      <div v-if="state === 'ready' && chartData" class="flex h-full flex-col">
        ...
        <div class="mt-4 flex flex-col items-center gap-2">
          <div v-if="topReason" class="text-xs font-bold text-slate-900 dark:text-white">
          ...
          <div class="flex flex-wrap justify-center gap-3">
            <div v-for="item in categories" :key="item.label" class="flex items-center gap-1.5 text-xs">
              ...
              <span class="text-slate-400 dark:text-slate-400">{{ item.label }} {{ item.count }}</span>
            </div>
          </div>
        </div>
      </div>
