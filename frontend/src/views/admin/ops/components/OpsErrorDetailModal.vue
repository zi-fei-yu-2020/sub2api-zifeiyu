  ...
  <BaseDialog :show="show" :title="title" width="full" :close-on-click-outside="true" @close="close">
    <div v-if="loading" class="flex items-center justify-center py-16">
      <div class="flex flex-col items-center gap-3">
        ...
        <div class="text-sm font-medium text-slate-400 dark:text-slate-400">{{ t('admin.ops.errorDetail.loading') }}</div>
      </div>
    </div>

    <div v-else-if="!detail" class="py-10 text-center text-sm text-slate-400 dark:text-slate-400">
      {{ emptyText }}
    </div>
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.requestId') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-slate-900 dark:text-white">
            {{ requestId || '—' }}
          </div>
        </div>

        <div class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.time') }}</div>
        ...
        <div class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.platform') }}</div>
          <div class="mt-1 text-sm font-medium text-slate-900 dark:text-white">
            {{ detail.platform || '—' }}
          </div>
        </div>
        ...
        <div class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.group') }}</div>
          <div class="mt-1 text-sm font-medium text-slate-900 dark:text-white">
        ...
        <div class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.model') }}</div>
          <div class="mt-1 text-sm font-medium text-slate-900 dark:text-white">
        ...
        <div class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.inboundEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-slate-900 dark:text-white">
        ...
        <div class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.upstreamEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-slate-900 dark:text-white">
        ...
        <div class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.status') }}</div>
          <div class="mt-1">
            <span :class="['inline-flex items-center rounded-xl px-2 py-1 text-xs font-black ring-1 ring-inset shadow-sm', statusClass]">
              {{ detail.status_code }}
            </span>
          </div>
        </div>
        ...
        <div class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.upstreamStatus') }}</div>
          <div class="mt-1">
            <span :class="['inline-flex items-center rounded-xl px-2 py-1 text-xs font-black ring-1 ring-inset shadow-sm', upstreamStatusClass]">
        ...
        <div class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.requestType') }}</div>
          <div class="mt-1 text-sm font-medium text-slate-900 dark:text-white">
        ...
        <div class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.message') }}</div>
          <div class="mt-1 break-words text-sm font-medium text-slate-900 dark:text-white" :title="rootCauseMessage">
            {{ rootCauseMessage || '—' }}
          </div>
        </div>

        <div v-if="detail.api_key_prefix" class="rounded-xl bg-slate-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ t('admin.ops.errorDetail.apiKeyPrefix') }}</div>
          <div class="mt-1 font-mono text-sm font-medium text-slate-900 dark:text-white">
      ...
      <div class="rounded-xl bg-slate-50 p-6 dark:bg-dark-900">
        <h3 class="text-sm font-black uppercase tracking-wider text-slate-900 dark:text-white">{{ t('admin.ops.errorDetail.diagnosticPayloads') }}</h3>
        <div v-if="!diagnosticPayloadSections.length" class="mt-4 text-sm text-slate-400 dark:text-slate-400">{{ t('common.noData') }}</div>
        <div v-else class="mt-4 space-y-4">
          <div v-for="section in diagnosticPayloadSections" :key="section.key">
            <div class="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-400">{{ diagnosticPayloadLabel(section.key) }}</div>
            <pre class="max-h-[520px] overflow-auto rounded-xl border border-slate-200 bg-white p-4 text-xs text-gray-800 dark:border-slate-800 dark:bg-slate-900 dark:text-gray-100"><code>{{ prettyJSON(section.value) }}</code></pre>
          </div>
        </div>
      </div>

      <!-- Upstream errors list (only for request errors) -->
      <div v-if="showUpstreamList" class="rounded-xl bg-slate-50 p-6 dark:bg-dark-900">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <h3 class="text-sm font-black uppercase tracking-wider text-slate-900 dark:text-white">{{ t('admin.ops.errorDetails.upstreamErrors') }}</h3>
          <div class="text-xs text-slate-400 dark:text-slate-400" v-if="correlatedUpstreamLoading">{{ t('common.loading') }}</div>
        </div>

        <div v-if="!correlatedUpstreamLoading && !correlatedUpstreamErrors.length" class="mt-3 text-sm text-slate-400 dark:text-slate-400">
          {{ t('common.noData') }}
        </div>
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-black text-slate-900 dark:text-white">
                #{{ idx + 1 }}
                <span v-if="ev.type" class="ml-2 rounded-xl bg-slate-100 px-2 py-0.5 font-mono text-[10px] font-bold text-slate-700 dark:bg-slate-800 dark:text-gray-200">{{ ev.type }}</span>
              </div>
              <div class="flex items-center gap-2">
                <div class="font-mono text-xs text-slate-400 dark:text-slate-400">
                  {{ ev.status_code ?? '—' }}
                </div>
                <button
                  type="button"
                  class="inline-flex items-center gap-1.5 rounded-xl px-1.5 py-1 text-[10px] font-bold text-primary-700 hover:bg-primary-50 disabled:cursor-not-allowed disabled:opacity-60 dark:text-primary-200 dark:hover:bg-dark-700"
            ...
            <div class="mt-3 grid grid-cols-1 gap-2 text-xs text-slate-600 dark:text-gray-300 sm:grid-cols-2">
              <div>
                <span class="text-slate-400">{{ t('admin.ops.errorDetail.upstreamEvent.status') }}:</span>
                <span class="ml-1 font-mono">{{ ev.status_code ?? '—' }}</span>
              </div>
              <div>
                <span class="text-slate-400">{{ t('admin.ops.errorDetail.upstreamEvent.requestId') }}:</span>
                <span class="ml-1 font-mono">{{ ev.request_id || ev.client_request_id || '—' }}</span>
              </div>
            </div>

            <div v-if="ev.message" class="mt-3 break-words text-sm font-medium text-slate-900 dark:text-white">{{ ev.message }}</div>

            <pre
              v-if="expandedUpstreamDetailIds.has(ev.id)"
              class="mt-3 max-h-[240px] overflow-auto rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-gray-800 dark:border-slate-800 dark:bg-dark-900 dark:text-gray-100"
...
function statusBadgeClass(code: number): string {
  ...
  return 'bg-slate-50 text-slate-700 ring-gray-600/20 dark:bg-gray-900/30 dark:text-slate-400 dark:ring-gray-500/30'
}
