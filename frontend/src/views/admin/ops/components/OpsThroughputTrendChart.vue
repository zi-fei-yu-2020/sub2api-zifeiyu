...
<template>
  <div class="flex h-full min-w-0 flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-slate-900 dark:ring-dark-700">
    <div
    ...
    >
      <h3 class="flex min-w-0 items-center gap-2 text-sm font-bold text-slate-900 dark:text-white">
      ...
      <div
        ...
        class="flex w-full min-w-0 flex-wrap items-center gap-2 text-xs text-slate-400 dark:text-slate-400 sm:w-auto sm:justify-end"
      >
        ...
        <template v-if="!props.fullscreen">
          <button
            type="button"
            class="inline-flex shrink-0 items-center rounded-xl border border-slate-200 bg-white px-2 py-1 text-[11px] font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-800 dark:bg-dark-900 dark:text-gray-300 dark:hover:bg-dark-800"
    ...
    <div v-if="(props.topGroups?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      <button
        ...
        :key="g.group_id"
        type="button"
        class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-[11px] font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-800 dark:bg-dark-900 dark:text-gray-200 dark:hover:bg-dark-800"
        @click="emit('selectGroup', g.group_id)"
      >
        <span class="max-w-[180px] truncate">{{ g.group_name || `#${g.group_id}` }}</span>
        <span class="text-slate-400 dark:text-slate-400">{{ formatNumber(g.request_count) }}</span>
      </button>
    </div>
    ...
    <div v-else-if="(props.byPlatform?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        ...
        <span class="text-slate-400 dark:text-slate-400">{{ formatNumber(p.request_count) }}</span>
      </button>
    </div>
    ...
    <div class="min-h-0 min-w-0 flex-1">
      ...
      <div v-else class="flex h-full items-center justify-center">
        <div v-if="state === 'loading'" class="animate-pulse text-sm text-slate-400">{{ t('common.loading') }}</div>
