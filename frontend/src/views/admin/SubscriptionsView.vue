          ...
          <div class="flex flex-1 flex-wrap items-center gap-3">
            ...
            >
              ...
              <button
                ...
                type="button"
                class="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-gray-300"
              ...
              <div
                ...
                class="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-xl border border-slate-200 bg-white shadow-lg dark:border-slate-800 dark:bg-slate-900"
              >
                <div
                  v-if="filterUserLoading"
                  class="px-4 py-3 text-sm text-slate-400 dark:text-slate-400"
                >
                  {{ t('common.loading') }}
                </div>
                <div
                ...
                <button
                  ...
                  class="w-full px-4 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-dark-700"
                >
                  <span class="font-medium text-slate-900 dark:text-white">{{ user.email }}</span>
                  <span class="ml-2 text-slate-400 dark:text-slate-400">#{{ user.id }}</span>
                </button>
              </div>
            </div>
          ...
          <div class="ml-auto flex flex-wrap items-center justify-end gap-3">
            ...
            <div class="relative" ref="columnDropdownRef">
              ...
              <div
                ...
                class="absolute right-0 z-50 mt-2 w-48 origin-top-right rounded-xl border border-slate-200 bg-white shadow-lg dark:border-slate-800 dark:bg-slate-900"
              >
                <div class="p-2">
                  <!-- User column mode selection -->
                  <div class="mb-2 border-b border-slate-200 pb-2 dark:border-slate-800">
                    <div class="px-3 py-1 text-xs font-medium text-slate-400 dark:text-slate-400">
        ...
        >
          ...
          <template #cell-usage="{ row }">
            <div class="min-w-[280px] space-y-2">
              ...
              <div
                ...
                "
                class="flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-50 to-teal-50 px-3 py-2 dark:from-emerald-900/20 dark:to-teal-900/20"
              >
                <span class="text-lg text-blue-600 dark:text-emerald-400">∞</span>
          ...
          <template #cell-expires_at="{ value }">
            <div v-if="value">
              ...
              >
                <div v-if="remainingExpiry" class="text-xs text-slate-400">
                  {{ remainingExpiry }}
                </div>
              </template>
            </div>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-xl p-1.5 text-slate-400 transition-colors hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-900/20 dark:hover:text-blue-400"
              >
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-xl p-1.5 text-slate-400 transition-colors hover:bg-orange-50 hover:text-orange-600 dark:hover:bg-orange-900/20 dark:hover:text-orange-400 disabled:cursor-not-allowed disabled:opacity-50"
              >
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-xl p-1.5 text-slate-400 transition-colors hover:bg-green-50 hover:text-primary-600 dark:hover:bg-green-900/20 dark:hover:text-green-400"
              >
                <Icon name="refresh" size="sm" />
        ...
        <div>
          ...
          >
            <template #selected="{ option }">
              ...
              />
              <span v-else class="text-slate-400">{{ t('admin.subscriptions.selectGroup') }}</span>
            </template>
    ...
    >
      ...
      >
        <div class="rounded-xl bg-slate-50 p-4 dark:bg-slate-800">
          ...
          <p v-if="extendingSubscription.expires_at" class="mt-1 text-sm text-slate-600 dark:text-slate-400">
      ...
      <transition name="modal">
        <div v-if="showGuideModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="showGuideModal = false">
          ...
          <div class="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-6 shadow-2xl dark:bg-slate-900">
            <button type="button" class="absolute right-4 top-4 text-slate-400 hover:text-slate-600 dark:hover:text-gray-200" @click="showGuideModal = false">
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>

            <h2 class="mb-4 text-lg font-bold text-slate-900 dark:text-white">{{ t('admin.subscriptions.guide.title') }}</h2>
            <p class="mb-5 text-sm text-slate-400 dark:text-slate-400">{{ t('admin.subscriptions.guide.subtitle') }}</p>
            ...
            <div class="mb-5">
              ...
              <div class="ml-8 overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700">
                <table class="w-full text-sm">
                  <tbody>
                    <tr v-for="(row, i) in guideActionRows" :key="i" class="border-b border-slate-100 dark:border-slate-800 last:border-0">
                      <td class="whitespace-nowrap bg-slate-50 px-3 py-2 font-medium text-slate-700 dark:bg-slate-800 dark:text-gray-300">{{ row.action }}</td>
                      <td class="px-3 py-2 text-slate-600 dark:text-slate-400">{{ row.desc }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <!-- Tip -->
            <div class="rounded-xl bg-blue-50 p-3 text-xs text-blue-700 dark:bg-blue-900/20 dark:text-blue-300">
...
.usage-label {
  @apply w-10 flex-shrink-0 text-xs font-medium text-slate-400 dark:text-slate-400;
}

.usage-amount {
  @apply whitespace-nowrap text-xs tabular-nums text-slate-600 dark:text-gray-300;
}
