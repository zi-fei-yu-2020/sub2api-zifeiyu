  ...
  <AppLayout>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-aff_code="{ row }">
            <span class="font-mono text-sm text-slate-700 dark:text-gray-300">{{ row.aff_code || '-' }}</span>
          </template>
          <template #cell-order="{ row }">
            <div class="space-y-0.5">
              <div class="font-mono text-sm text-slate-900 dark:text-white">#{{ row.order_id }}</div>
              <div class="max-w-56 truncate text-sm text-slate-400 dark:text-dark-400">{{ row.out_trade_no }}</div>
            </div>
          </template>
          ...
          <template #cell-pay_amount="{ row }">
            <span class="text-sm text-slate-900 dark:text-white">¥{{ formatAmount(row.pay_amount) }}</span>
          </template>
          ...
          <template #cell-created_at="{ row }">
            <span class="text-sm text-slate-700 dark:text-gray-300">{{ formatDateTime(row.created_at) }}</span>
          </template>
        </DataTable>
      </template>
    ...
    >
      ...
      <div v-else-if="selectedOverview" class="space-y-4">
        <div class="rounded-xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
          <div class="font-mono text-sm text-slate-900 dark:text-white">#{{ selectedOverview.user_id }}</div>
          <div class="mt-1 text-sm font-medium text-slate-900 dark:text-white">{{ selectedOverview.email || '-' }}</div>
          <div class="mt-0.5 text-sm text-slate-400 dark:text-dark-400">{{ selectedOverview.username || '-' }}</div>
        </div>
...
const UserCell = defineComponent({
  ...
  setup(cellProps, { emit }) {
    return () => h('div', { class: 'space-y-0.5' }, [
      h('div', { class: 'font-mono text-sm text-slate-900 dark:text-white' }, `#${cellProps.id}`),
      h(cellProps.clickable ? 'button' : 'div', {
        class: cellProps.clickable
          ...
          : 'max-w-56 truncate text-sm text-slate-700 dark:text-gray-300',
        type: cellProps.clickable ? 'button' : undefined,
        onClick: cellProps.clickable ? () => emit('open', cellProps.id) : undefined,
      }, cellProps.email || '-'),
      h('div', { class: 'max-w-56 truncate text-sm text-slate-400 dark:text-dark-400' }, cellProps.username || '-'),
    ])
  },
})
  ...
  setup(amountProps) {
    return () => h('span', {
      class: amountProps.strong
        ? 'text-sm font-semibold text-blue-600 dark:text-emerald-400'
  ...
  setup(amountProps) {
    return () => {
      ...
      if (value === null || value === undefined) {
        return h('span', { class: 'text-sm text-slate-400 dark:text-dark-500' }, '-')
      }
...
const OverviewStat = defineComponent({
  ...
  setup(statProps) {
    return () => h('div', { class: 'rounded-xl border border-slate-100 bg-white p-3 dark:border-slate-800 dark:bg-dark-900' }, [
      h('div', { class: 'text-sm text-slate-400 dark:text-dark-400' }, statProps.label),
      h('div', {
        class: statProps.mono
          ? 'mt-1 font-mono text-base font-semibold text-slate-900 dark:text-white'
          : 'mt-1 text-base font-semibold text-slate-900 dark:text-white',
      }, statProps.value),
    ])
  },
})
