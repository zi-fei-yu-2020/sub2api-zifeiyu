  ...
  <div ref="containerRef" class="relative">
    <div v-if="selectedUserIds.length > 0" class="mb-2 flex flex-wrap gap-2">
      <span
        ...
        :key="userId"
        class="inline-flex max-w-full items-center gap-1.5 rounded-xl bg-slate-100 px-2.5 py-1.5 text-xs text-slate-700 dark:bg-dark-600 dark:text-gray-200"
      >
        <span class="max-w-64 truncate font-medium" :title="selectedUserLabel(userId)">
          {{ selectedUserLabel(userId) }}
        </span>
        <span class="shrink-0 text-slate-400">#{{ userId }}</span>
        <span
        ...
        <button
          type="button"
          class="shrink-0 rounded text-slate-400 hover:text-red-600 dark:hover:text-red-400"
    ...
    >
      <div v-if="searchLoading" class="px-4 py-3 text-sm text-slate-400 dark:text-slate-400">
        {{ t("common.loading") }}
      </div>
      <div
      ...
      <template v-else>
        <button
          ...
          :key="user.id"
          type="button"
          class="flex w-full items-center justify-between gap-3 px-4 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-dark-600"
          @click="selectUser(user)"
        >
          <span class="min-w-0 truncate font-medium text-slate-900 dark:text-white">
            {{ user.email }}
            <span v-if="user.deleted" class="ml-1 text-xs font-normal text-slate-400">
              {{ t("admin.settings.openaiFastPolicy.userDeleted") }}
            </span>
          </span>
