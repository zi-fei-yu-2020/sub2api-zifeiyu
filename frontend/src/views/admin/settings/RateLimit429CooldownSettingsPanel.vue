      ...
      <template v-else>
        ...
        >
          <div>
            ...
            <input
              v-model.number="cooldownSecondsModel"
              type="number"
              min="1"
              max="7200"
              class="input w-32"
...
const emit = defineEmits<{
  ...
  'update:cooldownSeconds': [value: number]
  save: []
}>()
...
const cooldownSecondsModel = computed({
  get: () => props.cooldownSeconds,
  set: (value: number) => emit('update:cooldownSeconds', value),
})
