  ...
  <BaseDialog :show="show" :title="operation === 'add' ? t('admin.users.deposit') : t('admin.users.withdraw')" width="narrow" @close="$emit('close')">
    <form v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      <div class="flex items-center gap-3 rounded-xl bg-slate-50 p-4 dark:bg-slate-800">
        ...
        <div class="flex-1"><p class="font-medium text-slate-900 dark:text-gray-100">{{ user.email }}</p><p class="text-sm text-slate-400 dark:text-slate-400">{{ t('admin.users.currentBalance') }}: ${{ formatBalance(user.balance) }}</p></div>
      </div>
      <div>
        ...
        <div class="relative flex gap-2">
          <div class="relative min-w-0 flex-1">
            <span
              data-testid="balance-currency-prefix"
              aria-hidden="true"
              class="pointer-events-none absolute inset-y-px left-px z-10 flex w-10 items-center justify-center rounded-l-md border-r border-slate-200 bg-slate-50 font-medium text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400"
            >$</span>
            <input
              ...
              min="0"
              required
              class="input pl-12 tabular-nums"
            />
          </div>
      ...
      <div v-if="form.amount > 0" class="rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-950"><div class="flex items-center justify-between text-sm"><span class="text-slate-700 dark:text-gray-300">{{ t('admin.users.newBalance') }}:</span><span class="font-bold text-slate-900 dark:text-gray-100">${{ formatBalance(calculateNewBalance()) }}</span></div></div>
    </form>
    <template #footer>
