    ...
    <div v-if="oldGroup" class="space-y-4">
      ...
      <div class="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-900">
        <div class="flex items-center gap-2">
          ...
          <span class="font-medium text-slate-900 dark:text-white">{{ oldGroup.name }}</span>
          <Icon name="arrowRight" size="sm" class="ml-auto text-slate-400" />
      ...
      <div v-if="availableGroups.length > 0" class="max-h-64 space-y-2 overflow-y-auto">
        <label
          ...
          :class="selectedGroupId === group.id
            ...
            : 'border-slate-200 hover:border-slate-300 dark:border-slate-700 dark:hover:border-dark-500'"
        >
          <input
            type="radio"
            :value="group.id"
          ...
          <div class="flex-1">
            ...
            <span class="ml-2 text-xs text-slate-400">{{ group.platform }}</span>
          </div>
        </label>
      </div>
