  ...
  >
    <form id="refund-form" @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div
        ...
        class="rounded-xl border border-violet-200 bg-violet-50 p-3 dark:border-violet-800 dark:bg-violet-900/20"
      >
      ...
      <div class="rounded-xl bg-slate-50 p-3 dark:bg-slate-800">
        <div class="flex justify-between text-sm">
          <span class="text-slate-400 dark:text-slate-400">{{ t('payment.orders.orderId') }}</span>
          <span class="font-mono text-slate-900 dark:text-white">#{{ order?.id }}</span>
        </div>
        <div class="mt-1 flex justify-between text-sm">
          <span class="text-slate-400 dark:text-slate-400">{{ t('payment.orders.creditedAmount') }}</span>
          <span class="font-medium text-slate-900 dark:text-white">{{ creditedAmountSymbol }}{{ order?.amount?.toFixed(2) }}</span>
        </div>
        <div class="mt-1 flex justify-between text-sm">
          <span class="text-slate-400 dark:text-slate-400">{{ t('payment.orders.payAmount') }}</span>
          <span class="font-medium text-slate-900 dark:text-white">{{ paymentAmountSymbol }}{{ order?.pay_amount?.toFixed(2) }}</span>
        </div>
        <div v-if="actuallyRefunded > 0" class="mt-1 flex justify-between text-sm">
          <span class="text-slate-400 dark:text-slate-400">{{ t('payment.admin.alreadyRefunded') }}</span>
      ...
      <div>
        <div class="flex items-center gap-2">
          ...
          />
          <label for="deduct-balance" class="text-sm text-slate-700 dark:text-gray-300">
            {{ t('payment.admin.deductBalance') }}
          </label>
          <span class="text-xs text-slate-400 dark:text-slate-400">{{ t('payment.admin.deductBalanceHint') }}</span>
        </div>
        ...
        <div v-if="form.deduct_balance && userBalance != null" class="mt-3 grid grid-cols-2 gap-3">
          <div class="rounded-xl bg-slate-50 p-3 text-sm dark:bg-slate-800">
            <div class="text-slate-400 dark:text-slate-400">{{ t('payment.admin.userBalance') }}</div>
            <div class="mt-1 font-semibold text-slate-900 dark:text-white">{{ creditedAmountSymbol }}{{ userBalance.toFixed(2) }}</div>
          </div>
          <div class="rounded-xl bg-slate-50 p-3 text-sm dark:bg-slate-800">
            <div class="text-slate-400 dark:text-slate-400">{{ t('payment.admin.orderAmount') }}</div>
            <div class="mt-1 font-semibold text-slate-900 dark:text-white">{{ creditedAmountSymbol }}{{ order?.amount?.toFixed(2) }}</div>
          </div>
        </div>
        ...
        <div
          ...
          class="mt-2 rounded-xl bg-amber-50 p-3 text-sm text-amber-700 dark:bg-amber-900/20 dark:text-amber-300"
        >
        ...
        <div
          ...
          class="mt-2 rounded-xl bg-blue-50 p-3 text-sm text-blue-700 dark:bg-blue-900/20 dark:text-blue-300"
        >
      ...
      <div>
        ...
        <div class="relative">
          <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">{{ creditedAmountSymbol }}</span>
          <input
      ...
      <div
        v-if="warning"
        class="rounded-xl bg-yellow-50 p-3 text-sm text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-300"
      >
        {{ warning }}
      </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        <button
          ...
          class="rounded-xl bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:opacity-50 dark:focus:ring-offset-dark-800"
        >
