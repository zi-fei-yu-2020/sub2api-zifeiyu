<template>
  <BaseDialog :show="show" :title="t('admin.groups.rateMultipliersTitle')" width="wide" @close="handleClose">
    <div v-if="group" class="space-y-4">
      <!-- 分组信息 -->
      <div class="flex flex-wrap items-center gap-3 rounded-xl bg-slate-50 px-4 py-2.5 text-sm dark:bg-slate-800">
      ...
      <div class="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
        <!-- 添加用户 -->
        <h4 class="mb-2 text-sm font-medium text-slate-700 dark:text-gray-300">
        ...
        <div class="flex items-end gap-2">
          <div class="relative flex-1">
            ...
            <div
              ...
              class="absolute left-0 right-0 top-full z-10 mt-1 max-h-48 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-lg dark:border-dark-500 dark:bg-slate-800"
            >
              <button
                v-for="user in searchResults"
                :key="user.id"
                type="button"
                class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm hover:bg-slate-50 dark:hover:bg-dark-600"
                @click="selectUser(user)"
              >
                <span class="text-slate-400">#{{ user.id }}</span>
                <span class="text-slate-900 dark:text-white">{{ user.username || user.email }}</span>
        ...
        <!-- 批量调整 + 全部清空 -->
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center gap-3 border-t border-slate-100 pt-3 dark:border-slate-700">
          <span class="text-xs font-medium text-slate-400 dark:text-slate-400">{{ t('admin.groups.batchAdjust') }}</span>
          <div class="flex items-center gap-1.5">
            ...
            <input
              ...
              min="0"
              autocomplete="off"
              class="hide-spinner w-20 rounded border border-slate-200 bg-white px-2 py-1 text-center text-sm transition-colors focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500/20 dark:border-dark-500 dark:bg-slate-800 dark:focus:border-primary-500"
              placeholder="0.5"
            />
            <button
              type="button"
          ...
          <div class="ml-auto">
            <button
              type="button"
              class="rounded-xl border border-red-200 bg-red-50 px-3 py-1.5 text-sm font-medium text-red-600 transition-colors hover:bg-red-100 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40"
              @click="clearAllLocal"
            >
      ...
      <div v-else>
        ...
        <div v-if="localEntries.length === 0" class="py-6 text-center text-sm text-slate-400 dark:text-slate-400">
              ...
              <table class="w-full min-w-max text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-400 dark:text-slate-400">{{ t('admin.groups.columns.userEmail') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-400 dark:text-slate-400">ID</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-400 dark:text-slate-400">{{ t('admin.groups.columns.userName') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-400 dark:text-slate-400">{{ t('admin.groups.columns.userNotes') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-400 dark:text-slate-400">{{ t('admin.groups.columns.userStatus') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-400 dark:text-slate-400">{{ t('admin.groups.columns.rateMultiplier') }}</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-dark-600">
                  ...
                  >
                    <td class="px-3 py-2 text-slate-600 dark:text-slate-400">{{ entry.user_email }}</td>
                    <td class="whitespace-nowrap px-3 py-2 text-slate-400 dark:text-slate-400">{{ entry.user_id }}</td>
                    <td class="whitespace-nowrap px-3 py-2 text-slate-900 dark:text-white">{{ entry.user_name || '-' }}</td>
                    <td class="max-w-[160px] truncate px-3 py-2 text-slate-400 dark:text-slate-400" :title="entry.user_notes">{{ entry.user_notes || '-' }}</td>
                    ...
                    <td class="whitespace-nowrap px-3 py-2">
                      <input
                        ...
                        class="hide-spinner w-20 rounded border border-slate-200 bg-white px-2 py-1 text-center text-sm font-medium transition-colors focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500/20 dark:border-dark-500 dark:bg-slate-800 dark:focus:border-primary-500"
                    ...
                    <td class="px-2 py-2">
                      <button
                        type="button"
                        class="rounded p-1 text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
