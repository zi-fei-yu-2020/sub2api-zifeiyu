      ...
      <div v-else class="max-h-96 overflow-auto rounded-xl border border-slate-200 dark:border-slate-700">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-700">
          ...
          <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-slate-900">
            <tr v-for="profile in profiles" :key="profile.id" class="hover:bg-slate-50 dark:hover:bg-dark-700">
              <td class="px-3 py-2">
                <div class="font-medium text-slate-900 dark:text-white text-sm">{{ profile.name }}</div>
              </td>
              <td class="px-3 py-2">
                <div v-if="profile.description" class="text-sm text-slate-400 dark:text-slate-400 max-w-xs truncate">
                  {{ profile.description }}
                </div>
                <div v-else class="text-xs text-slate-400 dark:text-slate-600">—</div>
              </td>
              <td class="px-3 py-2">
                <Icon
                  :name="profile.enable_grease ? 'check' : 'lock'"
                  size="sm"
                  :class="profile.enable_grease ? 'text-green-500' : 'text-slate-400'"
                />
              </td>
              <td class="px-3 py-2">
                <div v-if="profile.alpn_protocols?.length" class="flex flex-wrap gap-1">
                  ...
                  <span v-if="profile.alpn_protocols.length > 3" class="text-xs text-slate-400">
