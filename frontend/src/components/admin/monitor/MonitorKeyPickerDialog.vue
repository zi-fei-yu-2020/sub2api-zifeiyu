<template>
  ...
  >
    <div class="space-y-3">
      ...
      <div v-else-if="filteredKeys.length === 0" class="py-6 text-center text-sm text-slate-400">
      ...
      <div v-else class="max-h-96 overflow-auto rounded-xl border border-slate-200 dark:border-slate-700">
        <table class="w-full text-sm">
          <thead class="bg-slate-50 dark:bg-slate-900 sticky top-0 z-10">
            <tr class="text-left text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-slate-400">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-dark-700">
            <tr
              ...
              :key="k.id"
              class="cursor-pointer hover:bg-slate-50 dark:hover:bg-dark-700"
              @click="$emit('pick', k)"
            >
              <td class="px-3 py-2 font-medium text-slate-900 dark:text-white">{{ k.name }}</td>
              <td class="px-3 py-2 font-mono text-xs text-slate-400 dark:text-slate-400">{{ maskApiKey(k.key) }}</td>
              <td class="px-3 py-2">
                <GroupBadge
                  v-if="k.group"
                  :name="k.group.name"
