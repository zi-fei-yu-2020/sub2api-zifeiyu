<template>
  ...
  >
    ...
    <div v-else-if="monitors.length === 0" class="py-6 text-center text-sm text-slate-400">
    ...
    <div v-else>
      ...
      <div class="mb-2 flex items-center gap-3 text-xs">
        ...
        <button
          type="button"
          class="text-slate-400 hover:underline dark:text-slate-400"
          @click="selectNone"
        >
          {{ t('admin.channelMonitor.template.selectNone') }}
        </button>
        <span class="ml-auto text-slate-400 dark:text-slate-400">
      ...
      <ul class="max-h-80 divide-y divide-gray-100 overflow-y-auto rounded-xl border border-slate-200 dark:divide-dark-700 dark:border-slate-800">
        <li
          v-for="m in monitors"
          :key="m.id"
          class="flex cursor-pointer items-center gap-3 px-3 py-2 hover:bg-slate-50 dark:hover:bg-dark-800"
          @click="toggle(m.id)"
        >
          <input
            type="checkbox"
          ...
          <span class="text-xs text-slate-400">{{ m.provider }}</span>
          <span v-if="m.provider === 'openai'" class="text-xs text-slate-400">{{ m.api_mode }}</span>
          <span
            v-if="!m.enabled"
            class="ml-auto rounded bg-slate-100 px-1.5 py-0.5 text-xs text-slate-400 dark:bg-slate-800 dark:text-slate-400"
          >
