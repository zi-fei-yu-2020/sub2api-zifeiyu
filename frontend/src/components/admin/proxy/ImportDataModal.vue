  ...
  >
    <form id="import-proxy-data-form" class="space-y-4" @submit.prevent="handleImport">
      ...
      <div>
        ...
        <div
          class="flex items-center justify-between gap-3 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-900"
        >
          <div class="min-w-0">
            <div class="truncate text-sm text-slate-700 dark:text-dark-200">
              {{ fileName || t('admin.proxies.dataImportSelectFile') }}
            </div>
            <div class="text-xs text-slate-400 dark:text-dark-400">JSON (.json)</div>
          </div>
