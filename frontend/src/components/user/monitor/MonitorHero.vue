<template>
  <section class="py-3 md:py-4">
    <div class="flex items-center justify-end gap-3 flex-wrap">
      <div
        role="tablist"
        class="inline-flex p-0.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 text-xs"
      >
        <button
          ...
          :class="window === opt.value
            ? 'bg-white dark:bg-slate-800 shadow-sm text-slate-900 dark:text-white font-semibold'
            : 'text-slate-400 hover:text-slate-700 dark:text-slate-400 dark:hover:text-gray-200'"
      ...
      <button
        type="button"
        class="h-8 w-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-gray-200 dark:hover:bg-dark-700 transition-colors disabled:opacity-50"
        :disabled="loading"
