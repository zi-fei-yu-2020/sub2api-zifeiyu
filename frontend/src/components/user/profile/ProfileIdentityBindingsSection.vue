          ...
          <div class="flex min-w-0 flex-1 items-start gap-4">
            ...
            <div class="min-w-0 flex-1 space-y-3">
              ...
              <div
                ...
                class="grid gap-1 text-sm text-slate-400 dark:text-slate-400"
              >
                <p
...
const rowClass = computed(() =>
  props.embedded
    ? compact.value
      ? 'rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-dark-900/40'
      : 'rounded-2xl border border-slate-100 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-dark-900/30'
    : 'px-6 py-5'
)
...
function providerIconClass(provider: UserAuthProvider): string {
  ...
  if (provider === 'wechat') {
    return 'bg-green-100 text-primary-600 dark:bg-green-900/20 dark:text-green-300'
  }
  if (provider === 'oidc') {
