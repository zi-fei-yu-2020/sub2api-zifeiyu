      ...
      <template v-if="notifyEnabled">
        ...
        <div>
          <label class="input-label">
            ...
            <span class="text-xs text-slate-400 ml-2">{{ t('profile.balanceNotify.thresholdHint') }}</span>
          </label>
        ...
        <div>
          ...
          <div v-if="emailEntries.length > 0" class="space-y-2 mb-3">
            <div v-for="(entry, idx) in emailEntries" :key="idx"
              ...
              <div class="flex items-center gap-2 min-w-0 flex-1">
                ...
                <span class="text-sm text-slate-700 dark:text-gray-300 truncate">{{ entry.email }}</span>
              </div>
                ...
                <template v-if="!entry.verified">
                  ...
                  <template v-if="verifyingEmail === entry.email">
                    <input
                      ...
                      type="text"
                      maxlength="6"
                      class="w-20 rounded border border-slate-300 px-2 py-1 text-xs dark:border-dark-500 dark:bg-slate-800"
                    ...
                    <span v-if="verifyCountdown > 0" class="text-xs text-slate-400">{{ verifyCountdown }}s</span>
                    <button v-else @click="sendCodeForSaved(entry.email)" :disabled="sendingSavedCode" class="text-xs text-slate-400 hover:text-slate-700">
                      {{ t('profile.balanceNotify.resend') }}
                    </button>
                    <button @click="verifyingEmail = ''" class="text-xs text-slate-400 hover:text-slate-600">
                      {{ t('common.cancel') }}
                    </button>
                  </template>
                  <template v-else>
          ...
          <div v-if="pendingEmails.length > 0" class="space-y-2 mb-3">
            <div v-for="(pe, idx) in pendingEmails" :key="pe.email"
              ...
              <span class="flex-1 text-sm text-slate-700 dark:text-gray-300">{{ pe.email }}</span>
              ...
              <div v-else class="flex items-center gap-1">
                ...
                <span v-if="pe.countdown > 0" class="text-xs text-slate-400">{{ pe.countdown }}s</span>
                <button v-else @click="sendCodeFor(idx)" :disabled="pe.sending" class="text-xs text-slate-400 hover:text-slate-700">
                  {{ t('profile.balanceNotify.resend') }}
                </button>
              </div>
            </div>
