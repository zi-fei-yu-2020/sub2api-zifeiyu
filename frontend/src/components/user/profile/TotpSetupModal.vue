<template>
  <div class="fixed inset-0 z-50 overflow-y-auto" @click.self="$emit('close')">
    <div class="flex min-h-full items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-md transform rounded-xl bg-white p-6 shadow-xl transition-all dark:bg-slate-900">
        <!-- Header -->
        <div class="mb-6 text-center">
          <h3 class="text-xl font-semibold text-slate-900 dark:text-white">
            {{ t('profile.totp.setupTitle') }}
          </h3>
          <p class="mt-2 text-sm text-slate-400 dark:text-slate-400">
            {{ stepDescription }}
          </p>
        </div>
        ...
        <div v-if="step === 1" class="space-y-6">
          ...
          <template v-if="setupData">
            <div class="flex justify-center">
              <div class="rounded-lg border border-slate-200 p-4 bg-white dark:border-slate-700 dark:bg-white">
                <img :src="qrCodeDataUrl" alt="QR Code" class="h-48 w-48" />
              </div>
            </div>

            <div class="text-center">
              <p class="text-sm text-slate-400 dark:text-slate-400 mb-2">
                {{ t('profile.totp.manualEntry') }}
              </p>
              <div class="flex items-center justify-center gap-2">
                <code class="rounded bg-slate-100 px-3 py-2 font-mono text-sm dark:bg-slate-800">
                  {{ setupData.secret }}
                </code>
                <button
                  type="button"
                  class="rounded p-1.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-dark-700"
                  @click="copySecret"
                >
            ...
            <div class="mb-6">
              ...
              <div class="flex justify-center gap-2">
                <input
                  ...
                  inputmode="numeric"
                  pattern="[0-9]"
                  class="h-12 w-10 rounded-lg border border-slate-300 text-center text-lg font-semibold focus:border-primary-500 focus:ring-primary-500 dark:border-slate-700 dark:bg-slate-800"
